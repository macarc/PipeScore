...
export default function render(dispatch: (e: ScoreEvent) => void, state: UIState): V {
  ...
  return h('div', [
    ...
    h('div', { id: 'sidebar', style: `width: ${state.width}px` }, [
      ...
      h('h3', ['Anacrusis']),
      h('button', { class: 'add text' }, { click: () => dispatch({ name: 'add anacrusis', before: true }) }, ['before']),
      h('button', { class: 'add text' }, { click: () => dispatch({ name: 'add anacrusis', before: false }) }, ['after']),
      h('button', { class: 'delete' }, { click: () => dispatch({ name: 'delete bar' }) }),
      h('hr'),
      h('h2', ['Stave']),
