...
export function totalWidth(notes: Note[], prevNote: Pitch | null): Width {
  return notes
...
function widthOfGracenote(note: Note, pitchBefore: Pitch | null) {
  return note.isTied()
    ? width.zero()
    : width.init(note.gracenote().width(note.pitch(), pitchBefore), 0);
}

export function noteWidth(note: Note, pitchBefore: Pitch | null): Width {
  return width.addAll(
...
function drawGracenote(note: Note, props: GracenoteProps) {
  return note.gracenote().render({
    ...props,
...
function drawHead(note: Note, x: number, y: number, props: NoteProps): m.Children {
  ...
  return m('g[class=note-head]', [
    ...
    m('rect', {
      ...
      opacity: 0,
      onmousedown: (e: MouseEvent) => dispatch(clickNote(note, e)),
...
function drawNatural(note: Note, x: number, y: number): m.Children {
...
function drawTails(note: Note, x: number, y: number) {
...
function layoutNotes(notes: Note[], props: NoteProps): NoteLayout {
...
export function drawNoteGroup(notes: Note[], props: NoteProps) {
