...
export const widthOfNote = (
  ...
  Note.isTriplet(note)
    ? [
        ...
        (acc, { n, p }) => width.add(acc, widthOfGracenote(n, p)),
        width.init(3 * noteHeadWidth, 3)
      )
    : width.add(
        width.init(noteHeadWidth, 1),
        Note.hasDot(note.length) ? width.init(5, 0) : width.init(0, 0),
        widthOfGracenote(note, prevNote)
      );

// Finds the total width of the note array in beat widths
const totalWidth = (notes: NoteModel[], prevNote: Pitch | null): Width =>
  notes
    .map((n, i) => widthOfNote(n, i === 0 ? prevNote : notes[i - 1].pitch))
    .reduce((a, b) => width.add(a, b), width.zero());

// Finds the offset that the note head has due to its gracenote
export const noteHeadOffset = (
  beatWidth: number,
  note: BaseNote,
...
function noteHead(
  ...
  const pointerEvents =
    draggedNote || gracenoteBeingDragged ? 'none' : 'visiblePainted';
...
M ${x1},${y1} S ${midx},${midloy}, ${x0},${y0}
    `
      : `
M ${x0},${y0} S ${x0 - noteWidth / 2},${y0 - tieHeight - tieWidth}, ${
          x0 - noteWidth
        },${y0}
...
): previous is PreviousNote =>
  note.tied && (previous || false) && previous.pitch === note.pitch;
...
function singleton(
  ...
  return svg('g', { class: 'singleton' }, [
    shouldTie(note, previousNote)
      ? tie(staveY, note.pitch, x, noteWidth, previousNote, lastStaveX)
      : null,
    shouldTie(note, previousNote)
      ? null
      : renderGracenote(note.gracenote, gracenoteProps),
...
function renderTriplet(triplet: TripletModel, props: NoteProps): V {
  ...
  const firstStemY = Note.hasBeam(triplet.length)
    ? Math.max(defaultHeight(triplet.first.pitch), secondY + 20)
    : defaultHeight(triplet.first.pitch);
  const thirdStemY = Note.hasBeam(triplet.length)
    ? Math.max(defaultHeight(triplet.third.pitch), secondY + 20)
    : defaultHeight(triplet.third.pitch);
  const secondStemY = Note.hasBeam(triplet.length)
    ? ((secondX - firstX) / (thirdX - firstX)) * (thirdStemY - firstStemY) +
      firstStemY
    : defaultHeight(triplet.second.pitch);
  ...
  const secondGracenoteProps = {
    ...
    y: props.y,
    noteWidth: props.noteWidth,
    thisNote: triplet.second.pitch,
    previousNote: triplet.first.pitch,
    dispatch: props.dispatch,
    state: props.gracenoteState,
  };
  const thirdGracenoteProps = {
    ...
    y: props.y,
    noteWidth: props.noteWidth,
    thisNote: triplet.third.pitch,
    previousNote: triplet.second.pitch,
    dispatch: props.dispatch,
    state: props.gracenoteState,
  };
  ...
  return svg('g', { class: 'triplet' }, [
    triplet.tied && props.previousNote
      ? tie(
          props.y,
          triplet.first.pitch,
          firstX,
  ...
  if (Note.isTriplet(group)) {
    ...
    if (group.length === 0) {
      ...
      const xOfGracenote = (noteIndex: number) =>
        props.x +
        width.reify(
          totalWidth(group.slice(0, noteIndex), previousPitch),
          props.noteWidth
        );
      const xOf = (noteIndex: number) => {
        const note = group[noteIndex];
        return (
          xOfGracenote(noteIndex) +
          (note.tied
            ? 0
            : width.reify(
                gracenoteWidth(
      ...
      const totalXWidth = width.reify(
        totalWidth(group, previousPitch),
        props.noteWidth
      );
      ...
      const setNoteXY = (note: NoteModel, index: number) =>
        setXY(
          note.id,
          xOfGracenote(index),
      ...
      if (Note.numberOfNotes(group) === 1) {
        ...
        const diff = cap(
          ...
          noteOffset(lastNote.pitch) - noteOffset(firstNote.pitch),
          30 / group.length
        );

        const [lowestNote, lowestNoteIndex, multipleLowest]: [
          NoteModel,
          number,
          boolean
        ] = group.reduce(
          (last, next, index) => {
            ...
            if (noteOffset(next.pitch) === noteOffset(lowestNoteSoFar.pitch)) {
              ...
              noteOffset(next.pitch) > noteOffset(lowestNoteSoFar.pitch)
            ) {
        ...
        return svg(
          ...
          group.map((note, index) => {
            ...
            const beforePreviousNoteTails =
              nmap(group[index - 2] || null, (n) =>
                Note.lengthToNumTails(n.length)
              ) || 0;
            const afterNoteTails =
              nmap(group[index + 1] || null, (n) =>
                Note.lengthToNumTails(n.length)
              ) || 0;
            ...
            const gracenoteProps = {
              ...
              thisNote: note.pitch,
              previousNote:
                nmap(previousNote, (p) => p.pitch) ||
                nmap(props.previousNote, (p) => p.pitch),
              dispatch: props.dispatch,
              state: props.gracenoteState,
            };
            const previousNoteObj =
              nmap(previousNote, (p) => ({
                pitch: p.pitch,
            ...
            return svg('g', { class: `grouped-note ${note.pitch}` }, [
              shouldTie(note, previousNoteObj)
                ? tie(
                    props.y,
                    note.pitch,
                    xOf(index),
              ...
              previousNote !== null && index > 0
                ? beamFrom(
                    ...
                    Note.lengthToNumTails(note.length),
                    Note.lengthToNumTails(previousNote.length),
                    beforePreviousNoteTails,
                    afterNoteTails
                  )
                : null,
