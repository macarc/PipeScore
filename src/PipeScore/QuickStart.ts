...
const blankForm = async (): Promise<ScoreOptions> => {
  const ts = await dialogueBox([
    ...
    h('label', [
      'Number of Staves:',
      h('input', { type: 'number', id: 'stave-number', min: 0, max: 16, value: 4, required: true })
    ]),
    h('label', [
      'Time Signature:',
      h('input', { type: 'number', id: 'num', min: 0, value: 2, style: 'display: block', required: true }),
  ...
  () => {
    ...
    if (nameElement && nameElement instanceof HTMLInputElement
        && stavesElement && stavesElement instanceof HTMLInputElement
        && numElement && numElement instanceof HTMLInputElement
        && denomElement && denomElement instanceof HTMLSelectElement
        && cutTimeElement && cutTimeElement instanceof HTMLInputElement) {
          return {
            name: nameElement.value,
            numberOfStaves: parseInt(stavesElement.value),
            timeSignature: TimeSignature.from(cutTimeElement.checked ? 'cut time' : [parseInt(numElement.value), TimeSignature.parseDenominator(denomElement.value) || 4])
          }
        }
