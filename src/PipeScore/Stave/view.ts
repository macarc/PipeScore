...
export default function render(stave: StaveModel, props: StaveProps): V {
  ...
  return svg('g', { class: 'stave' }, [
    ...
    svg('g', { class: 'bars' }, stave.bars.map((bar, idx) => renderBar(bar, barProps(bar, idx)))),
    svg('g', { class: 'stave-lines' }, staveLines.map(y => svg('line', { x1: props.x, x2: props.x + props.width, y1: y, y2: y, stroke: 'black', 'pointer-events': 'none' })))
  ]);
}
