...
const isValid = (st: SecondTimingModel, others: SecondTimingModel[]): boolean => {
  if (!(itemBefore(st.start, st.middle) && (itemBefore(st.middle, st.end) || st.middle === st.end))) {
    return false;
  }
  ...
  for (const other of others) {
    ...
    if (
         ...
         (st.start === other.end || st.end === other.start || st.start === other.start || st.end === other.end)
