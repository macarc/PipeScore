...
import { PreviousNote } from './NoteModel';
import TimeSignature from './TimeSignature';
import Bar, { BarModel, DisplayBar } from './Bar';
...
function prerender(stave: StaveModel, props: StaveProps): DisplayStave {
  ...
  const stavelines = [...Array(5).keys()].map(idx => lineHeightOf(idx) + props.y);
  ...
  const barProps = (bar: BarModel, index: number, previousNote: PreviousNote | null) => ({
    x: getX(index),
    y: props.y,
    width: barWidth,
    shouldRenderLastBarline: index === (stave.bars.length - 1),
    previousNote,
    previousTimeSignature: (stave.bars[index - 1] && stave.bars[index - 1].timeSignature) || null
  });
  const displayBars: DisplayBar[] = [];

  const previousNote: PreviousNote | null = null;
  for (let idx = 0, bar = stave.bars[idx]; idx < stave.bars.length; idx++) {
    displayBars.push(Bar.prerender(bar, barProps(bar, idx, previousNote)));
  }
...
function render(display: DisplayStave): Svg {
  return svg`
    <g class="stave">
      ${renderTrebleClef(display.left, display.y)}
      <g class="notes">
        ${display.bars.map(
...
const renderTrebleClef = (x: number, y: number) => svg`
