...
const noteAndGracenoteWidth = (notes, gracenoteRatio, prevNote=null) =>
	notes.map((n,i) => 1 +
...
const totalBeatWidth = (note,previousNote) => noteAndGracenoteWidth(note.notes, gracenoteToNoteWidthRatio, previousNote);

const lastNoteOfWholeNote = wholeNote => wholeNote ? wholeNote.notes[wholeNote.notes.length - 1].note : null;

const numberOfNotes = note => note.notes.length;
const beamFrom = (x1,y1, x2,y2, length1,length2) => {
	// draw beams from note1 at x1,y1 to note2 x2,y2
	// where note1 is length1 long and note2 is length2
	const leftIs1 = x1 < x2;
...
const noteHead = (x,y, note,noteIndex,selected, mousedown,mouseup) => {
    // Draw note head, ledger line and dot
    const noteWidth = 5;
    const noteHeight = 4;
    const rotation = 30;
...
const singleton = (note,noteIndex,lastNote, x,y, noteWidth,numberOfTails) => {
    const stemX = x - 5;
...
const render = (note,props) => {
    ...
    const relativeIndexOfGracenote = (note,index) => noteAndGracenoteWidth(note.notes.slice().splice(0,index), gracenoteToNoteWidthRatio, lastNote);
    ...
    if (numberOfNotes(note) === 1 && note.notes[0].note === 'R') {
      ...
      const numberOfTails = Math.ceil(-1 * Math.log(note.length) / Math.log(2));
      return singleton(note,0,lastNote,props.x,props.y,props.noteWidth,numberOfTails);
    } else {
      const cap = (n, cap) =>
        (n > cap) ? cap :
        (n < -cap) ? -cap :
      ...
      const diffForLowest = 30 + noteOffset(lowestNote.note) - (multipleLowest ? 0 : diff * relativeIndexOf(lowestNote) / totalBeatWidth(note,props.previousNote));
      ...
      const stemYOf = note => 
        props.y()
          + (multipleLowest()
            ...
            ? 0
            // otherwise use a slant
            : diff * relativeIndexOf(note) / totalBeatWidth(note,props.previousNote))
          // offset so that the lowest note is always a constant height
          + diffForLowest;
      // Intentional double equals (array out of bounds)
      const notANote = note => note == null || note.note === 'R';
          ...
          ${note.notes.map(
            ...
            (shortNote,index) => {
              ...
              return svg`<g class="grouped-note">
                  ...
                  ${/*todo*/noteHead(xOf(shortNote), yOf(shortNote), shortNote,props.selectedNotes.includes(index), e => click(index, e), unclick)}

                  ${
                    previousNote ? this.beamFrom(stemXOf(shortNote),stemYOf(shortNote), stemXOf(previousNote),stemYOf(previousNote), shortlength, previousNote.length) : null
                  }
